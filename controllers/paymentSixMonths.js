...
exports.startPayment = [isAuthenticated, async (req, res) => {
    ...
    request.requestBody({
        ...
        purchase_units: [{
            ...
            description: 'VIP 6 MOUTHS ORDER'
        }],
        application_context: {
            return_url: 'http://localhost:4050/api/paymentSixMonths/paypal-return',
            cancel_url: 'http://localhost:4050/api/paymentSixMonths/paypal-cancel',
        }
    });
...
exports.handleReturn = [isAuthenticated, async (req, res) => {
    ...
    if (order.result.status === 'APPROVED') {
        ...
        if (captureOrder.result.status === 'COMPLETED') {
            ...
            res.redirect(`/api/paymentSixMonths/confirm-payment?token=${token}`);
        } else {
            console.log(`Order status is ${captureOrder.result.status}, redirecting to cancel`);
            res.redirect(`/api/paymentSixMonths/paypal-cancel?token=${token}`);
        }
    ...
    if (order.result.status === 'COMPLETED') {
        ...
        try {
            ...
            const result = await sequelizeCsgoVip.query(
                ...
                { replacements: [steamID, uniqueCode, '6 months', dateNow] } 
            )
        ...
        console.log(`Order status is ${order.result.status}, redirecting to cancel`);
        res.redirect(`/api/paymentSixMonths/paypal-cancel?token=${tokenId}`);
    }
}];
